...
# 모델 파일 경로
# - 우선순위: Keras(.keras, 학습 당시 export) > PyTorch(.pt, train_fall.py가 저장)
# - 어떤 것도 존재하지 않으면 휴리스틱(MediaPipe + YOLO fallback)만 사용한다.
...
def _get_model():
    """LSTM 모델 로드 — .keras → .pt → 휴리스틱.

    - 두 모델 파일이 모두 없거나 로드에 실패하면 None 을 반환하고,
      이후 호출에서는 더 이상 재시도하지 않는다 (로그 한 번만).
    ...
    if os.path.exists(_KERAS_MODEL_PATH):
        try:
            ...
            _lstm_backend = "keras"
            print(f"[FallDetector] Keras LSTM 모델 로드 완료: {_KERAS_MODEL_PATH}")
            return _lstm_model
        except Exception as e:
            print(f"[FallDetector] Keras 모델 로드 실패 — PT 폴백 시도: {e}")
    ...
    if os.path.exists(_PT_MODEL_PATH):
        try:
            ...
            print(
                f"[FallDetector] PyTorch LSTM 모델 로드 완료: {_PT_MODEL_PATH} "
                f"(window_size={win})"
            )
            return _lstm_model
        except Exception as e:
            print(f"[FallDetector] PyTorch 모델 로드 실패 — 휴리스틱만 사용: {e}")

    # 3) 모델 파일 없음 → 휴리스틱만
    print(
        "[FallDetector] LSTM 모델 파일이 없어 휴리스틱(MediaPipe Pose + YOLO)만 사용합니다.\n"
